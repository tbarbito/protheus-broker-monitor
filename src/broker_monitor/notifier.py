...
def send_alert(
    ...
    subject, body = _build_email(
        quarantined, restarted, failed, skipped, broker_unreachable
    )
...
def _build_email(
    ...
    if broker_unreachable:
        subject = f"[BROKER MONITOR] Broker inacessivel - {ts}"
        content = "<p>O monitoramento nao conseguiu acessar a pagina de status do Broker.</p><p>Verifique se o servico do Broker esta no ar.</p>"
    else:
        subject = f"[BROKER MONITOR] {len(quarantined)} slave(s) em quarentena - {ts}"
